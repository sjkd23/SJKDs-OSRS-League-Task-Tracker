...
function validatePayload(raw) {
  ...
  for (const rawSec of obj.s) {
    ...
    totalItems += sec.i.length;
    if (totalItems > 2e3) return null;
  }
  return obj;
}
...
var onRequestPost = /* @__PURE__ */ __name(async (context) => {
  ...
  if (contentLength > 524288) {
    return jsonResponse2({ error: "Payload too large." }, 413);
  }
  let bodyText;
  try {
    bodyText = await request.text();
  } catch {
    return jsonResponse2({ error: "Could not read request body." }, 400);
  }
  ...
  if (!validated) {
    return jsonResponse2({ error: "Invalid route payload." }, 400);
  }
  ...
  await env.ROUTE_SHARES.put(id, stored, { expirationTtl: 31536e3 });
  const origin = new URL(request.url).origin;
  const url = `${origin}/?r=${id}`;
  return jsonResponse2({ id, url });
}, "onRequestPost");

// ../.wrangler/tmp/pages-XQg1UD/functionsRoutes-0.8912628605723676.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-RC1G9B/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-RC1G9B/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.6534316358643071.mjs.map
